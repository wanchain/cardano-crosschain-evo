...
import Prelude hiding((&&),(==),($),(!!),(<),(>),(/=),(.),negate,length,filter,map,fst,snd,mconcat,any,not,foldMap)
...
-- scriptOutputsAt' :: ValidatorHash -> BuiltinByteString -> TxInfo' -> Bool ->[(Datum, Value)]
-- scriptOutputsAt' h stk p bCheckDatum =
--     let flt TxOut'{txOutDatum'=d, txOutAddress'=Address (ScriptCredential s) stk', txOutValue'} | s == h && (stakeCredentialToBytes stk') == stk = 
--         if bCheckDatum then 
--             case (PlutusTx.unsafeFromBuiltinData @OutputDatum d) of
